...
function draw() {
  ...
  // When line moves off canvas, reappear on other side
  // at a new height

  for (let i = 0; i < linesArray.length; i++) {
    // move to the right
    linesArray[i].x += linesArray[i].dx;
    // if lines move up or down or none
    linesArray[i].y += linesArray[i].dy;

    if (linesArray[i].x > cnv.width) {
      // move to the right of canvas
      linesArray[i].x = -linesArray[i].length;
  ...
  // You Try:
  // a) gives the lines a new color when they reappear
  // b) Add 95 more lines
  // c) adjust the speed so no lines are moving "too slow"
  // d) get creative; come up with an animation of your own that uses these lines
